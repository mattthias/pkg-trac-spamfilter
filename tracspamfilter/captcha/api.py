...
class CaptchaSystem(Component):
    ...
    def test(self, req, author, content, ip):
        if not self._expired(req):
            return self.karma_points, N_('Human verified via CAPTCHA')
    ...
    def pre_process_request(self, req, handler):
        if req.path_info == '/captcha' and req.method == 'POST':
            ...
            elif req.args['captcha_response'] == exp:
                valid = True
    ...
    def _cleanup(self):
        ...
        if last+self.captcha_cleantime < tim:
            #self.log.warn('Cleanup captcha %s+%s < %s' % (last,self.captcha_cleantime, tim))
